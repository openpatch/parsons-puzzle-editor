...
class ParsonsUI {
  ...
  private updateRequireDragging(): void {
    ...
    const maxDistractors: number = parseInt(
      this.container.find("#max-distractors").val() as string,
      10,
    );
    const maxDistractorsVal: number = !Number.isNaN(maxDistractors)
      ? maxDistractors
      : 10;
  ...
  private bindEvents(): void {
    ...
    this.container.on("click", "#add-test", (event: Event) => {
      event.preventDefault();
      const graderContainer: Cash = $(
        event.currentTarget as HTMLElement,
      ).closest(".grader-form-container");
      const isVarTests: boolean = graderContainer.hasClass(
        "variable-check-grader-container",
      );
      const testList: Cash = graderContainer.find(".tests-list");
      testList.append(
        isVarTests ? render.renderVarTest() : render.renderUnitTest(),
      );
    });
    this.container.on("click", ".action.duplicate", (event: Event) => {
      ...
      const isUnitTest = $this
        .closest(".grader-form-container")
        .hasClass("unit-test-grader-container");
      const currentTest: Cash = $(event.currentTarget as HTMLElement).closest(
        ".test-container",
      );
      if (isUnitTest) {
        render
          .renderUnitTest(collectUnitTest(currentTest))
          .insertAfter(currentTest);
      } else {
        render
          .renderVarTest(collectVariableTest(currentTest))
          .insertAfter(currentTest);
      }
    });
    ...
    this.container.on("change", "#disable-indent", (event: Event) => {
      ...
      const commonSettingsContainer: Cash = $this.closest(
    ...
    this.container.on("click", "#generate-model-turtle", (event: Event) => {
      ...
      const executableCode = getValueFromEditor(
        this.container.find("#executable-code"),
      );

      const code = executableCode || solutionCode;
      setValueToEditor(
        this.container.find("#turtle-model-code"),
        code.replace(/myTurtle/g, "modelTurtle"),
      );
    });
    ...
    this.container.on("click", "#require-dragging", (event: Event) => {
      ...
      const hasDistractors = /.*?[^\s]/.test(getValueFromEditor(distractorsTa));
      const isDraggingChecked = this.container
        .find("#require-dragging")
        .is(":checked");
      const maxDistractors: number = parseInt(
        this.container.find("#max-distractors").val() as string,
        10,
